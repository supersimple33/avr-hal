...
use arduino_hal::hal::{delay, usb};
use arduino_hal::{delay_ms, prelude::*};
use arduino_hal::{port::{mode::{Input, Output, PullUp}, Pin}, usb::SuspendNotifier};
use arduino_hal::hal::usb::{AvrGenericUsbBus, AvrUsbBus};
...
fn main() -> ! {
    ...
    let trigger = pins.d4.into_pull_up_input();
    ...
    let usb_bus: &UsbBusAllocator<AvrUsbBus> = unsafe {
        static mut USB_BUS: Option<UsbBusAllocator<AvrUsbBus>> = None;
        &*USB_BUS.insert(AvrGenericUsbBus::with_suspend_notifier(usb, pll))
    };
    let int = usb_bus.interface();
    let a = Some(usb_bus.interrupt::<usb_device::endpoint::Out>(64, 1));
    ufmt::uwriteln!(&mut serial, "First interrupt is on!\r").unwrap_infallible();
    // let b = Some(usb_bus.interrupt::<usb_device::endpoint::In>(64, 2));

    // let hid_class: HIDClass<AvrUsbBus> = HIDClass::new(usb_bus, KeyboardReport::desc(), 1);
    ...
    // ufmt::uwriteln!(&mut serial, "Hello from 3!\r").unwrap_infallible();
