...
use usb_device::{bus::PollResult, bus::UsbBus, UsbDirection, UsbError, class_prelude::UsbBusAllocator, endpoint::{EndpointAddress, EndpointType}, Result as UsbResult};
use avr_device::{
	asm::delay_cycles,
...
pub struct EndpointTableEntry { // REVIEW: what should the scoping be here?
	pub is_allocated: bool, // REVIEW: i dont think this should be pub
	pub eptype_bits: u8,
	pub epdir_bit: bool,
	pub epsize_bits: u8,
}
					...
					let mut index_list = self
						...
						.filter(|(index, ep)| {
							if (!ep.is_allocated) && max_packet_size <= $ENDPOINT_MAX_BUFSIZE[*index] {
								...
								panic!("bad {} good", (!ep.is_allocated) && max_packet_size <= $ENDPOINT_MAX_BUFSIZE[*index]);
								false
							}
						});
					let index = index_list.next().map(|(index, _)| index).ok_or(UsbError::EndpointOverflow);
					// if let Err(err) = index {
					// 	panic!("not good at all");
					// } else {
					// 	panic!("early");
					// }
					// if (index == 1 && _interval == 1) || (index == 2 && _interval == 2) {
					// 	panic!("truth {} value {} index {} left {} right", tv, index, (!ep.is_allocated), max_packet_size <= $ENDPOINT_MAX_BUFSIZE[index]); // schrodingers error
					// }
